...
class BatteryMode {
  ...
  async init() {
    ...
    const waitForConnection = async () => {
      const connectionState = await this.adapter.getStateAsync(
        `${this.adapter.namespace}.info.connection`,
      );

      if (connectionState?.val === true) {
        this.adapter.log.debug(
          "Connection is active. Starting battery calibration.",
        );
    ...
    this.calibrationCheckInterval = setInterval(
      waitForConnection,
      checkInterval,
    );
  }
  ...
  async handleCalibration(deviceId) {
    ...
    try {
      if (calibrationProgress === "down") {
        this.adapter.log.debug(
          "Battery Calibration Step 1: Setting SOC to 0%.",
        );
        ...
        this.adapter.log.debug(
          "Battery Calibration Step 2: Setting SOC to 100%.",
        );
        await applySocValue(this.adapter, deviceId, 99, "minSOC");
        await applySocValue(this.adapter, deviceId, 100, "maxSOC");
      } else {
        this.adapter.log.warn(
          "Invalid calibration progress state. Skipping calibration.",
        );
      }
    } catch (error) {
      this.adapter.log.error(
        `Error during battery calibration: ${error.message}`,
      );
    }
  }
