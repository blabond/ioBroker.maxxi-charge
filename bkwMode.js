...
class BkwMode {
  ...
  async handleSOCChange(id, state) {
    ...
    // Check preconditions
    if (
      this.adapter.config.bkw_enable !== true ||
      this.adapter.config.batterycalibration === true
    ) {
      return;
    }
    ...
    if (!connectionState || connectionState.val !== true) {
      this.adapter.log.debug(
        "bkwMode: Connection is not active. Skipping SOC handling.",
      );
      return;
    }
    ...
    if (targetValue !== null) {
      ...
      await this.commands.sendCommandWithRetry(
        ipAddress,
        "baseLoad",
        dummyState,
        deviceId,
      ...
      this.adapter.log.debug(
        `bkwMode: baseLoad set to ${targetValue} W (SOC=${soc}%).`,
      );
    }
  }
