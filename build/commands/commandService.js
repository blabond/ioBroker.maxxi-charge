...
class CommandService {
    ...
    async applyDeviceSetting(deviceId, commandId, rawValue, options = {}) {
        ...
        const sendSucceeded = await this.sendCommandWithRetry(ipAddress, commandId, normalizedValue, normalizedDeviceId, options.source);
        if (!sendSucceeded) {
            return false;
        }
        await this.stateManager.setStateIfChanged(`${normalizedDeviceId}.sendcommand.${commandId}`, normalizedValue, true);
        return true;
    }
    dispose() {
    ...
    async sendCommandWithRetry(ipAddress, commandId, value, deviceId, source) {
