...
function readRequestBody(request, maxBytes) {
  return new Promise((resolve, reject) => {
    ...
    request.on("error", (error) => {
      rejectOnce(
        createHttpError(`Request stream error: ${error.message}`, 400),
      );
    });
  ...
  async handleRequest(request, response) {
    ...
    try {
      const rawBody = await readRequestBody(
        request,
        constants_1.LOCAL_API_BODY_LIMIT_BYTES,
      );
      let payload;
      try {
      ...
      const remoteIp = (0, helpers_1.normalizeIpAddress)(
  ...
  async forwardPayloadToCloud(rawBody, contentType) {
    try {
      await this.requestClient.post(
        constants_1.LOCAL_API_CLOUD_MIRROR_URL,
        rawBody,
        {
          headers: {
            "Content-Type":
