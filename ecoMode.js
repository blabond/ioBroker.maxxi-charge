...
class EcoMode {
  ...
  async init() {
    ...
    if (!this.winterFrom || !this.winterTo) {
      this.adapter.log.warn(
        "EcoMode: Winter dates not configured properly. Initialization aborted.",
      );
      return;
    }
    ...
    const waitForDevice = async () => {
      ...
      if (deviceId) {
        this.adapter.log.debug(
          `EcoMode: Active device found: ${deviceId}. Starting immediate evaluation.`,
        );
        ...
        attempts++;
        this.adapter.log.debug(
          "EcoMode: No active device found yet. Retrying...",
        );
      }
    };
  ...
  async evaluateSeason() {
    ...
    if (!deviceId) {
      this.adapter.log.debug(
        "EcoMode: No active device found. Skipping evaluation.",
      );
      return;
    }
    ...
    if (this.isExactWinterTo(today)) {
      this.adapter.log.debug(
        "EcoMode: Today is the Winter end date. Applying summer mode.",
      );
    ...
    if (this.isInWinterRange(today)) {
      this.adapter.log.debug(
        "EcoMode: Winter range active. Setting minSOC to 60.",
      );
      ...
      if (this.adapter.config.batterycalibration === true) {
        this.adapter.log.debug(
          "EcoMode: Battery Calibration aktiv. No action required.",
        );
      } else {
  ...
  async handleSOCChange(id, state) {
    ...
    if (!deviceId) {
      this.adapter.log.error(
        `Invalid state ID: ${id}. Unable to extract deviceId.`,
      );
      return;
    }
    ...
    if (
      this.adapter.config.enableseasonmode &&
      state.val >= 55 &&
      !this.adapter.config.batterycalibration
    ) {
  ...
  async applySummerOnce(deviceId) {
    ...
    if (!deviceId) {
      this.adapter.log.warn(
        "EcoMode: No active deviceId found for summer mode application.",
      );
      return;
    }
