  ...
  async onReady() {
    try {
      ...
      this.cleanupInterval = this.setInterval(
        () => this.cleanupActiveDevices(),
        validateInterval(30 * 1000),
      );

      // EcoMode initialisieren, falls aktiviert
      if (
        this.config.enableseasonmode &&
        this.config.batterycalibration === false
      ) {
        await this.ecoMode.init();
      }

      if (
        this.config.batterycalibration &&
        this.config.batterycalibration === true
      ) {
