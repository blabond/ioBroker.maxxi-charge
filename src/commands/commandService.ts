...
const COMMAND_DEFINITIONS: readonly CommandDefinition[] = [
    { id: 'maxOutputPower', name: { en: 'Micro-inverter maximum power (W)', de: 'Mikrowechselrichter maximale Leistung (W)' }, type: 'number', role: 'level', min: 300, max: 2300, unit: 'W' },
    { id: 'offlinePower', name: { en: 'Offline output power (W)', de: 'Offline-Ausgangsleistung (W)' }, type: 'number', role: 'level', min: 50, max: 600, unit: 'W' },
    { id: 'baseLoad', name: { en: 'Adjust output (W)', de: 'Ausgabe anpassen (W)' }, type: 'number', role: 'level', min: -1000, max: 600, unit: 'W' },
    { id: 'offlineMode', name: { en: 'Cloudservice', de: 'Cloudservice' }, type: 'number', role: 'value', min: 1, max: 2, states: { 1: 'Cloud mode aktiv', 2: 'Lokal mode aktiv' } },
    { id: 'threshold', name: { en: 'Response tolerance (W)', de: 'Reaktionstoleranz (W)' }, type: 'number', role: 'level', min: 3, max: 50, unit: 'W' },
    { id: 'minSOC', name: { en: 'Minimum battery discharge', de: 'Minimale Batterieentladung' }, type: 'number', role: 'level.min', min: 0, max: 99, unit: '%' },
    { id: 'maxSOC', name: { en: 'Maximum battery discharge', de: 'Maximale Batterieentladung' }, type: 'number', role: 'level.max', min: 20, max: 100, unit: '%' },
];

export default class CommandService {
    private readonly commandDefinitions = new Map<CommandId, CommandDefinition>(COMMAND_DEFINITIONS.map(definition => [definition.id, definition]));
    ...
    public async handleStateChange(id: string, state: StateChange): Promise<boolean> {
        if (!state || state.ack) return false;
        const parsedState = this.parseCommandStateId(id);
        if (!parsedState) return false;
        await this.applyDeviceSetting(parsedState.deviceId, parsedState.commandId, state.val, { source: 'stateChange' });
        return true;
    }

    public async applyDeviceSetting(deviceId: string, commandId: CommandId, rawValue: unknown, _options: { source?: string } = {}): Promise<boolean> {
        const normalizedDeviceId = normalizeDeviceId(deviceId);
    ...
    public dispose(): Promise<void> {
        for (const relativeId of this.subscribedStateIds) this.adapter.unsubscribeStates(relativeId);
    ...
    public handleDeviceInactive(deviceId: string): void {
        ...
        for (const relativeId of [...this.subscribedStateIds]) {
            if (!relativeId.startsWith(relativeIdPrefix)) continue;
            this.adapter.unsubscribeStates(relativeId);
            this.subscribedStateIds.delete(relativeId);
        }

        for (const commandId of this.commandDefinitions.keys()) this.confirmedCommandValueCache.delete(this.getConfirmedCommandValueCacheKey(normalizedDeviceId, commandId));
    }
    ...
    private async resetSendcommandFolder(deviceId: string): Promise<void> {
        ...
        for (const relativeId of [...this.subscribedStateIds]) {
            if (!relativeId.startsWith(`${channelId}.`)) continue;
            this.adapter.unsubscribeStates(relativeId);
            this.subscribedStateIds.delete(relativeId);
        }

        for (const commandId of this.commandDefinitions.keys()) this.confirmedCommandValueCache.delete(this.getConfirmedCommandValueCacheKey(deviceId, commandId));
        await this.adapter.delObjectAsync(channelId, { recursive: true });
    }

    private async sendCommandWithRetry(ipAddress: string, commandId: CommandId, value: number, deviceId: string): Promise<boolean> {
        ...
        for (let attempt = 1; attempt <= COMMAND_RETRY_COUNT + 1; attempt++) {
            try {
                this.adapter.log.debug(`CommandService: Sending ${commandId}=${value} to ${deviceId} via ${url} (attempt ${attempt}/${COMMAND_RETRY_COUNT + 1}).`);
                ...
                if (attempt <= COMMAND_RETRY_COUNT) {
                    this.adapter.log.warn(`CommandService: Retry ${attempt}/${COMMAND_RETRY_COUNT} for ${commandId} on ${deviceId}.`);
    ...
    private normalizeValue(definition: CommandDefinition, rawValue: unknown): number | null {
        ...
        if (clampedValue !== roundedValue) this.adapter.log.warn(`CommandService: Clamped ${definition.id} from ${roundedValue} to ${clampedValue}.`);
        return clampedValue;
    }
    ...
    private parseCommandStateId(fullId: string): { deviceId: string; commandId: CommandId } | null {
        ...
        if (parts.length !== 3 || parts[1] !== 'sendcommand') return null;
    ...
    private async hasConfirmedCommandValue(stateId: string, deviceId: string, commandId: CommandId, targetValue: number): Promise<boolean> {
        ...
        if (cachedValue === targetValue) return true;
