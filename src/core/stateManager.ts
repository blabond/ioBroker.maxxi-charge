import type { AdapterInstance, JsonValue, ManagedStateCommon, ObjectDefinition } from '../types/shared';
import {
    areValuesEqual,
...
function createComparableExistingObject(existingObject: ioBroker.Object, desiredDefinition: ObjectDefinition): string {
    ...
    for (const key of Object.keys(desiredDefinition.common)) {
        comparableCommon[key] = (existingObject.common as Record<string, unknown>)[key];
    }

    for (const key of Object.keys(desiredDefinition.native)) {
        comparableNative[key] = (existingObject.native as Record<string, unknown>)[key];
    }
...
export default class StateManager {
    ...
    public async ensureChannel(id: string, common: ioBroker.ChannelCommon): Promise<void> {
    ...
    public async ensureFolder(id: string, common: ioBroker.OtherCommon): Promise<void> {
        await this.ensureObject(id, {
            type: 'folder',
            common,
            native: {},
    ...
    public async ensureStateObject(id: string, common: ManagedStateCommon): Promise<void> {
        await this.ensureObject(id, {
            type: 'state',
            common,
            native: {},
    ...
    public async setStateIfChanged(id: string, value: ioBroker.StateValue, ack = true): Promise<boolean> {
        ...
        if (existingState && existingState.ack === ack && areValuesEqual(existingState.val, value)) {
    ...
    public async syncDevicePayload(deviceId: string, payload: unknown): Promise<void> {
        await this.ensureDevice(deviceId);
        await this.syncPayloadRecursive(deviceId, payload);
    }

    public async syncSettingsPayload(deviceId: string, payload: unknown): Promise<void> {
        await this.ensureDevice(deviceId);
    ...
    private async ensureObject(id: string, definition: ObjectDefinition): Promise<void> {
    ...
    private async syncPayloadRecursive(parentId: string, payload: unknown): Promise<void> {
    ...
    private normalizeStateValue(value: JsonValue | undefined): ioBroker.StateValue | undefined {
    ...
    private toSettableObject(definition: ObjectDefinition): ioBroker.SettableObject {
    ...
    private toPartialObject(definition: ObjectDefinition): ioBroker.PartialObject {
        switch (definition.type) {
            case 'device':
                return definition;
            case 'channel':
