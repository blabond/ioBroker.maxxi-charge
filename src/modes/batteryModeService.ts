...
import { BATTERY_CALIBRATION_EMPTY_SOC, BATTERY_CALIBRATION_FULL_SOC } from '../constants';
...
export default class BatteryModeService {
    ...
    public async handleSocChange(id: string, state: StateChange): Promise<void> {
        ...
        if (this.config.calibrationProgress === 'down' && state.val <= BATTERY_CALIBRATION_EMPTY_SOC) {
        ...
        if (this.config.calibrationProgress === 'up' && state.val >= BATTERY_CALIBRATION_FULL_SOC) {
    ...
    private async updateCalibrationState(enabled: boolean, progress: 'down' | 'up'): Promise<boolean> {
