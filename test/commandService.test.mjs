...
describe('CommandService', () => {
    function createEnvironment({ initializedState, withSendcommandChannel = true, existingStates = {} } = {}) {
        ...
        const adapter = {
            ...
            getObjectAsync: async id =>
                objects.has(id) ? { _id: id, type: id.endsWith('sendcommand') ? 'channel' : 'state' } : null,
            getStateAsync: async id =>
                states.has(id)
                    ? typeof states.get(id) === 'object' && states.get(id) !== null && 'val' in states.get(id)
                        ? states.get(id)
        ...
        return {
            createService: () => new CommandService(adapter, stateManager, requestClient),
            deletedObjects,
            states,
            subscribeCalls,
            unsubscribeCalls,
    ...
    it('resets sendcommand on update and skips the reset on the next reload', async () => {
        ...
        environment.deletedObjects.should.deep.equal([['ccu1.sendcommand', { recursive: true }]]);
        environment.states.get('ccu1._sendcommandInitialized').should.deep.equal({ val: '260410', ack: true });
